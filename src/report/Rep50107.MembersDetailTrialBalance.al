...
report 50107 "Members Detail Trial Balance"
{
    Caption = 'Member - Detail Trial Balance';
    DefaultLayout = RDLC;
    RDLCLayout = './xLayout/MembersDetailTrialBal.rdl';
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            column(MembersDetailTrialBalCaption; MembersDetailTrialBalCaptionLbl)
            {
            }
            ...
            begin
                ...
                // Get report for only Members Customers
                SetFilter("Customer Posting Group", 'MEM*');
            end;
        }
    }
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'New Page per Member';
                        ToolTip = 'Specifies if each Member''s information is printed on a new page if you have chosen two or more Members to be included in the report.';
                    }
                    // field(ExcludeCustHaveaBalanceOnly; ExcludeBalanceOnly)
                    // {
                    //     ApplicationArea = Basic, Suite;
                    //     Caption = 'Exclude Members That Have a Balance Only';
                    //     MultiLine = true;
                    //     ToolTip = 'Specifies if you do not want the report to include entries for Members that have a balance but do not have a net change during the selected time period.';
                    // }
                }
            }
        }
    ...
    var
        ...
        MembersDetailTrialBalCaptionLbl: Label 'Member - Detail Trial Balance';
        PageNoCaptionLbl: Label 'Page';
        AllAmtsLCYCaptionLbl: Label 'All amounts are in LCY';
        RepInclCustsBalCptnLbl: Label 'This report also includes Members that only have balances.';
