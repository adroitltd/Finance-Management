report 50100 "LPO Report"
{
    Caption = 'Local Purchase Order';
    DefaultRenderingLayout = "LocalPurchaseOrder.docx";
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            RequestFilterHeading = 'Local Purchase Order';
            ...
            dataitem("Purchase Line"; "Purchase Line")
            {
                ...
                column(LineTotal; Quantity * "Direct Unit Cost")
                {
                    AutoFormatExpression = "Purchase Header"."Currency Code";
                    AutoFormatType = 1;
                }
    ...
    rendering
    {
        layout("LocalPurchaseOrder.docx")
        {
            Type = Word;
            LayoutFile = './xLayout/LocalPurchaseOrder.docx';
            Caption = 'Local Purchase Order (Word)';
            Summary = 'The Local Purchase Order (Word) provides a basic layout.';
        }
    }
        ...
        if ApprovalEntry.FindSet() then begin
            repeat begin
                if ApprovalEntry.Status = ApprovalEntry.Status::Approved then begin
                    Sender := ApprovalEntry."Sender ID";
                    Approver := ApprovalEntry."Approver ID";
                end;
            end until ApprovalEntry.Next() = 0;
        end;
    end;
